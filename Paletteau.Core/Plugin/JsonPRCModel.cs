...
/* We basically follow the Json-RPC 2.0 spec (http://www.jsonrpc.org/specification) to invoke methods between Paletteau and other plugins, 
 ...
 *      Paletteau -------JsonRPCServerRequestModel--------> client
 *      Paletteau <------JsonRPCQueryResponseModel--------- client
 *      
 * When execute a action (which mean user select an item in reulst item):
 *      Paletteau -------JsonRPCServerRequestModel--------> client
 *      Paletteau <------JsonRPCResponseModel-------------- client
 * 
 */
...
namespace Paletteau.Core.Plugin
{
    ...
    /// <summary>
    /// Json RPC Request that Paletteau sent to client
    /// </summary>
    ...
    /// <summary>
    /// Json RPC Request(in query response) that client sent to Paletteau
    /// </summary>
    ...
    /// <summary>
    /// Represent the json-rpc result item that client send to Paletteau
    /// Typically, we will send back this request model to client after user select the result item
    /// But if the request method starts with "Paletteau.", we will invoke the public APIs we expose.
    /// </summary>
